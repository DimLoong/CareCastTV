...
之后**永久免登录**：每次打开应用会自动为登录状态续期 400 天（浏览器内核对 cookie
的有效期上限），只要电视一年内开过一次机，登录就永不过期。
...
CareCastTV 的播放源是**苹果 CMS（maccms V10）采集站 API**，特征是形如
`https://示例站.com/api.php/provide/vod` 的接口地址（程序会自动拼 `?ac=videolist` 等参数）。

> ⚠️ 注意区分：网上流传的「饭太硬 / 肥猫 / 王小二」等地址是 **TVBox 格式的聚合配置文件**，
> 供 TVBox/OK影视 等 App 使用，里面大量条目依赖 jar 爬虫，**不能直接填进本项目**。
> 不过 TVBox 配置文件内部 `sites` 数组里 `"type": 1` 且 `api` 是
> `http(s)://.../api.php/provide/vod` 形式的条目，本质就是苹果 CMS 接口，
> 把它的 `api` 地址抄出来即可在本项目使用。
...
**方法一：配置文件批量导入（推荐）** —— `/admin` → 「配置文件」粘贴 JSON 保存：
...
```json
{
  "api_site": {
    "demo1": {
      "name": "示例资源站A",
      "api": "https://示例站A.com/api.php/provide/vod"
    },
    "demo2": {
      "name": "示例资源站B",
      "api": "https://示例站B.com/api.php/provide/vod",
      "detail": "https://示例站B.com"
    }
  }
}
```

也可在「配置文件」填一个**订阅地址**（指向托管在网上的上述 JSON），并开启自动更新。

**方法二：单个添加** —— `/admin` → 「视频源配置」→ 添加视频源，逐个填 key/name/api。

配好后到 `/search` 搜任意片名验证：有结果即源可用；无结果说明源失效或格式不对。
